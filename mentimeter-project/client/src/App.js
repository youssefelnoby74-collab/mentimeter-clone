...
const [currentResultIndex, setCurrentResultIndex] = useState(0);
  ...
  const submitAllAnswers = () => {
    ...
    const missingAnswer = answers.some(
      (item) => !item.answer || !String(item.answer).trim()
    );
        ...
        {questionResult.options.map((option) => {
          ...
          return (
            <div key={option} style={{ marginBottom: "16px" }}>
              <div style={{ marginBottom: "8px", color: darkMode ? "white" : theme.text }}>
                {option} — {data.percent}% ({data.count} votes)
              </div>
              <div
                style={{
                  width: "100%",
                  height: darkMode ? "28px" : "22px",
                  background: darkMode ? "rgba(255,255,255,0.15)" : "#e2e8f0",
                  borderRadius: "999px",
                  overflow: "hidden"
                }}
              >
          ...
          <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
            ...
            <div style={questionsGrid}>
              {hostResults.map((questionResult, qIndex) =>
                renderResultBox(questionResult, qIndex, true)
              )}
            </div>
          </div>
        )}
        ...
        {page === "create" && (
          <>
            ...
            {questionType === "wordcloud" && (
              <p style={{ color: theme.muted }}>
                Participants will type a short answer.
              </p>
            )}
                ...
                <div style={questionsGrid}>
                  {questionsList.map((item, index) => (
                    <div key={index} style={questionBox}>
                      ...
                      <p style={{ color: theme.muted }}>
                        Type: {item.type === "wordcloud" ? "Word Cloud" : item.type}
                      </p>
